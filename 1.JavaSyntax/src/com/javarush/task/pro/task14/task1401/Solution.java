...
public class Solution {
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 5; i++) { // передаем в метод нового пользователя 5 раз
            addUser(new User());
        }

        User userToSearch = new User(); // создаем пользователя для поиска
        userToSearch.setName("Рафаэль"); // устанавливаем ему имя

        findUserIndex(userToSearch); // передаем пользователя в метод
    }

    static void addUser(User user) { // вводим имя
        System.out.print(INPUT_NAME);
        String name = scanner.nextLine();

        System.out.printf(INPUT_AGE, name); // вводим возраст
